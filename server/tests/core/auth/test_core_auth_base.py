...
from server.app.core.auth.auth_service import create_access_token, get_payload
...
async def test_create_access_token(jwt_token):
    ...
    assert ast.literal_eval(base64.b64decode(token_parts[0]).decode("utf-8")) == {
        "alg": "HS256",
        "typ": "JWT",
    }
...
async def test_get_payload(jwt_token):
    cred = HTTPAuthorizationCredentials(credentials=jwt_token, scheme="Bearer")
