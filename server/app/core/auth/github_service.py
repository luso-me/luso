...
from server.app.core.auth.auth_service import create_access_token
...
from server.app.core.user.model.base import UserRead, UserCreate
...
async def get_github_user(github_user) -> UserRead:
    ...
    if len(users) == 0:
        ...
        log.error(
            f"More than one user found with the same github " f"id users: {users}"
        )
        raise GithubCredentialsException()
