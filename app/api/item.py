...
import app.crud.item as item_db
...
async def update_item(item_id: ItemId, item: Item):
    if update_result := await item_db.update_one(item_id, item):
...
async def delete_item(item_id: ItemId):
    delete_result = await item_db.delete_one(item_id)
