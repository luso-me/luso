<script lang="ts">
  ...
  let openUserSkillUsedDialog = false;
  let openDeleteUserSkillConfirmationDialog = false;
  ...
  onMount(async () => {
    ...
    _populateExistingSkill();
  });

  function _populateExistingSkill() {
    let existingUserSkill: UserSkill = user.skills.find(f => f.skill_id === $params.skillid);

    userSkill.skill_id = existingUserSkill.skill_id;
    userSkill.user_rating = existingUserSkill.user_rating;
    userSkill.notes = existingUserSkill.notes;
    userSkill.used = existingUserSkill.used;
  }

  const onSubmit = async () => {
    user.skills.map((us, i) => {
      if (us.skill_id == userSkill.skill_id) {
        user.skills[i].user_rating = userSkill.user_rating;
        user.skills[i].notes = userSkill.notes;
        user.skills[i].used = userSkill.used;
      }
    });
  ...
  const onDelete = async () => {
    user.skills = user.skills.filter(s => s.skill_id !== userSkill.skill_id);
  ...
  function onSubmitUserSkillUsed() {
    ...
    openUserSkillUsedDialog = false;
  }

  function onUserSkillUsedEdit(used: UserSkillUsed) {
    userSkillUsed = used;
    openUserSkillUsedDialog = true;
  }
...
<h1 class="ml-4">Edit User Skill</h1>
    ...
    <form on:submit|preventDefault={onSubmit}>
      ...
      <Card>
        <Content>
          <h4>Details</h4>
          <div>
            <Textfield value="{skill.name} ({skill.category})" disabled label="Name"
      ...
      <CardActions>
        <Button on:click$preventDefault={() => (openDeleteUserSkillConfirmationDialog = true)}
                variant="raised">
...
<Dialog bind:open={openUserSkillUsedDialog}
        fullscreen
  ...
  <DialogContent id="content">
    ...
    <div>
      <Textfield bind:value={userSkillUsed.to_date} label="To" input$maxlength={100}
                 class="luso-dialog-field-with-helper" type="date">
        <svelte:fragment slot="helper">
          <HelperText>When did you stop using this skill? If you are still using it, just leave
            this
            blank.
          </HelperText>
        </svelte:fragment>
...
<Dialog bind:open={openDeleteUserSkillConfirmationDialog}
        fullscreen
        aria-labelledby="mandatory-title"
        aria-describedby="mandatory-content">

  <Header>
    <Title id="title">Delete Skill</Title>
    <IconButton action="close" class="material-icons">close</IconButton>
  </Header>

  <DialogContent id="content">
    <div>
      Are you sure you want to delete {skill.name} from your profile?
    </div>
  </DialogContent>

  <Actions>
    <Button on:click$preventDefault={() => (openDeleteUserSkillConfirmationDialog = false)}
            variant="raised">
